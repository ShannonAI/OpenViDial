...
MODEL_DIR="/data/yuxian/train_logs/video/images/layer${LAYER}_lr${LR}_bsz128_drop${DROPOUT}_warmup${WARMUP}"
mkdir -p $MODEL_DIR
# data directory
...
fairseq-train \
  ...
  --keep-last-epochs 5 \
  --ddp-backend=no_c10d \
  --num-workers 24  # Reading img-objects mmap-file could be very slow, so we recommend to use a large number of workers.
