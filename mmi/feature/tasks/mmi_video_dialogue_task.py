...
class MMIVideoDialogueTask(FairseqTask):
    ...
    def get_span_info(self, sent_num: np.array, split) -> np.array:
        """
        ...
        if (split == 'train' and self.random_list is None):
            print("======================")
            self.random_list = {}
            start_idx = 0
            for group_idx in range(sent_num.shape[0]):
                ...
                while (end_ <= start_idx+num-1):
                    ...
                    while (neg_idx == end_):
                        neg_idx = random.randint(0, max_num-1)
                    self.random_list[end_-1] = neg_idx
                    end_ += 1
                start_idx += num
        ...
        for group_idx in range(sent_num.shape[0]):
            ...
            while (end_ <= start_idx+num-1):
                ...
                if (split == 'train'):
                    span_idxs.append((0, end_-1, self.random_list[end_-1]))
                end_ += 1
            start_idx += num
