...
class MMI(nn.Module):
    ...
    def forward(self, src_text, text_len, src_image, src_image_mask):
        ...
        image_mask_tensor = torch.tensor(src_image_mask, dtype=torch.float, device=self.device)
        ...
        image_tensor = image_tensor.permute(0, 2, 1) # batch_size * max_obj * feature -> batch_size * feature * max_obj
        output = torch.nn.functional.sigmoid(torch.matmul(text_feature, image_tensor)) # batch_size * sen_len * max_obj
        image_out_mask_tensor = torch.unsqueeze(image_mask_tensor, dim=1)
        image_out_mask_tensor = image_out_mask_tensor.expand(image_out_mask_tensor.shape[0], output.shape[1], image_out_mask_tensor[2]) # batch_size * sen_len * max_obj
        output = output * image_out_mask_tensor
        output = output.sum(dim=-1) * text_mask # batch_size * sen_len
        len_tensor = torch.tensor(text_len, dtype=torch.float, device=self.device)+image_mask_tensor.sum(dim=-1)
